...
# Steps (How it Works):
# 1. Get a script of all the HTML on the page.
# 2. Get an article list - a snippet of the HTML outline including all the articles.
...
# Method Purpose: Finds the start and end of all correct articles hyperlinks in the previously extracted content string.
...
# 3. Each element of the end_indices list is incremented by 5 to get the last character of the hyperlink.
...
def find_occurrences(content_string):
    start_indices = [i for i in range(len(content_string)) if
                     content_string.startswith('https://www.nytimes.com/2024', i)]
...
# Method Purpose: Extracts all articles hyperlinks from the content string.
# Parameters: The starting and ending indices of the hyperlinks in the content string.
# Steps (How it Works):
# 1. Define an empty url_list which will be used for the aggregation of the URL themselves.
# 2. Use a for-range loop to get all the indices of start_indices list.
# Side Note: Since the lists are EQUALIZED in length from the previous method, the lengths of both will be equal.
# 3. Get a substring from the content string with the starting and ending index of the hyperlink.
